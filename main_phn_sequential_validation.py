...
from utils_moo import init_phn_output, validate_one_epochv2, write_training_phn_progress, compute_loss
from utils_moo import compute_spread_loss, generate_params, solve_one_batch, generate_rays
from utils_moo import solve_one_batchv2
...
def train_one_batch(agent, phn, phn_opt, batch, writer, num_ray=16, ld=1, is_initialize=False):
    ...
    logprob_list, f_list, sum_entropies_list = solve_one_batchv2(agent, phn, ray_list, batch)
    with torch.no_grad():
        agent.eval()
        _, greedy_f_list, _ = solve_one_batchv2(agent, phn, ray_list, batch)
...
def run(args):
    ...
    for epoch in range(last_epoch, args.max_epoch):
        ...
        if epoch % 5 == 0:
            validate_one_epochv2(args, agent, phn, validator, validation_dataset,test_batch,test_sample_solutions, writer, epoch) 
