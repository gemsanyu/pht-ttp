...
def train_one_epoch(agent, agent_opt, train_dataset, writer, entropy_loss_alpha=0.05):
    agent.train()
    ...
    for batch_idx, batch in tqdm(enumerate(train_dataloader), desc="step", position=1):
        ...
        agent.eval()
        with torch.no_grad():
            _, _, _, total_profits, critic_costs, _, _ = solve(agent, env)
        agent.train()
        ...
        write_training_progress(tour_lengths.mean(), total_profits.mean(), total_costs.mean(), agent_loss.detach(), entropy_loss.detach(), critic_costs.mean().detach(), logprobs.detach().mean(), env.num_nodes, env.num_items, writer)
