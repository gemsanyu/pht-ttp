...
def get_utility(n=10, device=CPU_DEVICE):
    n = float(n)
    idx = torch.arange(n, dtype=torch.float32, device=device) + 1
...
def get_hv_contributions(solution_list, num_random_points=1000000, reference_point=None, device=CPU_DEVICE):
    ...
    random_points = torch.rand((num_random_points, M), dtype=torch.float32, device=device)
    total_hv = get_hypervolume_mc(solution_list, random_points, reference_point=reference_point, device=device)
    if num_solutions == 1:
        return total_hv
    ...
    for i in range(num_solutions):
        ...
        hv_without_sol = get_hypervolume_mc(
            solution_list[solution_mask, :], random_points, reference_point=reference_point, device=device)
...
def get_nondominated_rank(score_list):
    ...
    for front in fronts:
        crowding_distance_list[front] = get_crowding_distance(score_list[front], fmax, fmin)

    cmp_new = functools.partial(cmp, front_idx=front_idx, crowding_distance_list=crowding_distance_list)
