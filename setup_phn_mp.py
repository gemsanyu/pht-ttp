...
@torch.no_grad()
def trace_model(agent: Agent, phn: PHN)->Tuple[Agent, Encoder]:
    agent.eval()
    device = agent.device
    ...
    ray_list = [torch.tensor([[float(i)/10,1-float(i)/10]]) for i in range(10)]
    ray = ray_list[0]
...
def trace_and_save_agent(args, phn):
...
def setup_phn_mp(args, load_best=False):
    encoder, phn, test_batch = setup_master(args, load_best)
    return encoder, phn, test_batch
