...
@torch.no_grad()
def test(agent, phn, test_env, x_file, y_file, prof, n_solutions=10):
    agent.eval()
    phn.eval()
...
def run(args):
    ...
    with torch.profiler.profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler('./runs/profiler_old/'),
        record_shapes=True,
        profile_memory=True,
        with_stack=True
    ) as prof, open(x_file_path.absolute(), "a+") as x_file, open(y_file_path.absolute(), "a+") as y_file:
        test(agent, phn, test_env, x_file, y_file, prof)
